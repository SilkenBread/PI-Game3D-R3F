...
export default function World4FOp(props) {
  ...
  const [visibleFloorPlarform, setVisibleFloorPlarform] = useState([
    false,
    false,
    false,
    false,
  ...
  const [materialColor, setMaterialColor] = useState("red");
  const onCollisionPlatform = () => {
    setMaterialColor("#083A00");
    setVisibleFloorPlarform(() => {
      return visibleFloorPlarform.map(() => true);
    });
  };

  const onCollisionOutPlatform = () => {
    setVisibleFloorPlarform(() => {
      setMaterialColor("red");
      return visibleFloorPlarform.map(() => false);
    });
  };
  ...
  return (
    <>
      <group {...props} dispose={null}>
        {plataformas.map(
          (mesh, index) =>
            visibleFloorPlarform[index] && (
              <RigidBody key={mesh.name} type="fixed" colliders={"trimesh"}>
                <mesh
        ...
        <RigidBody
          type="fixed"
          colliders ="cuboid"
          position={[-11.00, 13, -59.00]}
          onCollisionEnter={() => onCollisionPlatform()}
          onCollisionExit={()=> onCollisionOutPlatform()}
        ...
        <RigidBody
          type="fixed"
          colliders="cuboid"
          onCollisionEnter={(e) => limitsCollision(e)}
        >
          <mesh
        ...
        />
        <mesh
          geometry={nodes.Blades_5.geometry}
          material={materials.terColor}
        />
      </group>
    </>
  );
