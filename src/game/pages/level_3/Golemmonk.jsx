...
export default function Golemmonk({ positions, ...props }) {
  const { nodes, materials } = useGLTF('assets/models/characters/rockVilllan.glb')
  const ref = useRef();
  ...
  useFrame((state, delta) => {
    if (positions.length > 0 && currentIndex < positions.length && chasing == false) {
      targetPosition = positions[currentIndex];
      const currentPosition = ref.current.position;
      const direction = new THREE.Vector3().subVectors(targetPosition, currentPosition);
      const distance = direction.length();
      const velocity = direction.normalize().multiplyScalar(velocityVar * delta); // Ajusta la velocidad aquí
    ...
    } else if (positions.length > 0 && currentIndex < positions.length && chasing == true) {
  ...
  return (<>
    ...
    >
      <CylinderCollider
        args={[1.6, 2, 1.6]} // Radius, height, and depth of the cylinder collider
        position={[0, 0.5, 0]} // Relative position of the collider within the RigidBody
        onCollisionEnter={(e) => {
